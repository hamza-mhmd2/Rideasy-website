...
const DriverApp = () => {
    ...
    const fetchRideHistory = async () => {
      try {
        const response = await axios.get(
          `${process.env.REACT_APP_REMOTE_URL}/api/v1/rides/ride-history`,
          {
            headers: {
  ...
  useEffect(() => {
    const setDriverActive = async (driverId, lat, lng) => {

      console.log(`set driver active in : ${driverId}, ${lat}, ${lng}`);
      ...
      let config = {
        ...
        url: `${process.env.REACT_APP_REMOTE_URL}/api/v1/rides/set-driver-active`,
        headers: {
      ...
      try {
       ...
       console.log('setDriverActive response', resp.data)
    ...
    navigator.geolocation.watchPosition(
      (position) => {
        ...
        setDriverActive(locationData.driverId, locationData.lat, locationData.lng)
  ...
  return (
    <div className="min-h-screen bg-gray-100 flex flex-col items-center p-4">
      <div className="flex justify-between w-full max-w-6xl mb-4">
        ...
        <nav>
          <Link to="/driver/inbox" className="text-lg text-blue-600 mx-2">
            Inbox
          </Link>
          <Link to="/driver/rides" className="text-lg text-blue-600 mx-2">
            View Rides
          </Link>
        </nav>
      </div>
      ...
      {isModalOpen && selectedRide && (
        <div className="fixed inset-0 flex items-center justify-center z-50 bg-black bg-opacity-50">
          <div className="bg-white rounded-lg p-6 w-full max-w-lg">
            ...
            <p>
              <strong>Title:</strong> {selectedRide.title}
            </p>
            <p>
              <strong>Date:</strong>{" "}
